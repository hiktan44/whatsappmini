...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "video/*", "audio/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/plain"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for pywhatapp-media" ON storage.objects FOR SELECT USING (bucket_id = 'pywhatapp-media');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for pywhatapp-media" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'pywhatapp-media');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for pywhatapp-media" ON storage.objects FOR UPDATE USING (bucket_id = 'pywhatapp-media');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for pywhatapp-media" ON storage.objects FOR DELETE USING (bucket_id = 'pywhatapp-media');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'pywhatapp-media',
        public: true,
        allowed_mime_types: ["image/*", "video/*", "audio/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/plain"],
