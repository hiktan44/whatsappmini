...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "video/*", "audio/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for whatsapp-media" ON storage.objects FOR SELECT USING (bucket_id = 'whatsapp-media');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for whatsapp-media" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'whatsapp-media');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for whatsapp-media" ON storage.objects FOR UPDATE USING (bucket_id = 'whatsapp-media');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for whatsapp-media" ON storage.objects FOR DELETE USING (bucket_id = 'whatsapp-media');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'whatsapp-media',
        public: true,
        allowed_mime_types: ["image/*", "video/*", "audio/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"],
